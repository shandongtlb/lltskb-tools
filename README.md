...
- 🟢 **车辆交路表**（**离线**，全网 1.6 万+ 车次 / 3600+ 交路链，含普速 K/Z/T + 动车 G/D/C；口语化车辆描述另抽**规范车型码**列 25G/CR400BF-S/CRH2A…）
- **车型图片 + 车型详情表**（复兴号/和谐号全型号：整车外观 + 各车厢平面 + 座位实拍，及编组/定员/时速/设施汇总 CSV）
...
| `crawl_images.py` | 下载**车型图片** + 自动汇总**车型详情.csv**（编组/长度/定员/时速/餐车/设施车厢）；`--no-img` 只出数据不下图；副产物落 **车次车型.csv**（车次→实际车型，零额外请求） | 12306 `getCarDetail` 接口 + 图床 |
| `join_cartype.py` | **离线合表**：把 `车次车型.csv`（12306 实际车型）join 到 `车次查交路.csv`（离线交路+描述抽取车型），出**对照表** 描述车型 vs 实际车型 + 是否一致 | 本地两张 CSV（零联网） |
...
├── latest/               # 最新一版
│   ├── jlb.dat           # 原始交路表二进制
│   ├── 车次交路.csv       # 交路链 | 车辆描述 | 车型 | 车次数
│   ├── 车次查交路.csv     # 车次 → 所在交路链 | 车辆描述 | 车型
...
- **`车型详情.csv`**（脚本同级，跑完自动汇总）—— 一行一车型：`车型, 类型, 样本车次, 样例车底, 车厢数, 列车编组, 列车长度, 列车定员, 最高时速, 餐车, 静音车厢, 婴儿护理台, 列车办公席, 无障碍车厢, 座位类型, 编组明细`。其中 **`座位类型`**（各座别及官方描述，如"商务座#包间式结构…/一等座#…/二等座#…"）与 **`编组明细`**（每节车厢座位分布，如"01车 商务/一等 6/28 \| 02车 二等 90 \| …"）来自接口返回的 `coachDetailPicList`/`coachPicList`，本就在 `_info.json` 里，零额外请求。
- **`车次车型.csv`**（副产物，脚本同级）—— `车次, 车型, 类型, 当日车底, 查询日期`：接口本就为每趟车返回了车型，去重时被丢掉，这里零额外请求顺手落表，供 §4 离线合到交路表。

> 断网现场只要车型数据表：有网时跑 `--no-img` 出一张小 CSV 拷进去即可。`runningDay` 默认取**当天**，无需手改；若某些车次当天不开导致漏采，加 `--day 20260710` 指定次日。

### 4. 交路 × 实际车型 对照表（离线合表）

`crawl_images.py --all` 跑完得到 `车次车型.csv` 后，**离线**把它 join 到交路表（不再联网）：

```bash
python3 join_cartype.py     # 读 latest/车次查交路.csv + 车次车型.csv → 车次车型对照.csv
```

输出 `车次车型对照.csv`：`车次 | 所在交路链 | 描述抽取车型(离线) | 实际车型(12306) | 类型 | 当日车底 | 是否一致`。

```
C20   C19#C20#C21#C22   CR400AF   CR200J3-C   复兴号   CR200J3-C-1136   ✗
G2    G43#G42#…#G2      CR400BF-Z CR400BF-Z   复兴号   CR400BF-Z-…      ✓
```

- **`是否一致` 列筛 `✗`** 即可揪出路路通交路描述与 12306 实际车底对不上的车次（多为**换代**如 25G→CR200J、换底、描述滞后）——反过来是校正交路描述的线索。
- `getCarDetail` 只覆盖动车组 G/D/C，**普速无实际车型**→该列留空、以离线描述抽取为准。
- 「实际车型」是**查询当日的快照**，会随换底/检修变动；「描述抽取车型」是路路通相对静态的配属。二者定位不同，不是谁替代谁。
