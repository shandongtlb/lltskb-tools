...
**规范车型码抽取（`parse_jlb.extract_cartype`）：** 车辆描述口语化（`25G型,DC600V,双管供风,集便`），另抽一列规范车型码便于按车型筛。正则三支：`CRH\d{1,3}[A-Z]{0,2}(?:-[A-Z]{1,2})?`（和谐号）、`CR\d{3}[A-Z]{1,2}\d?(?:-[A-Z]{1,2})?`（复兴号，含 CR200J3-C）、`2[0-9][A-Z]{1,2}`（25 系普速）。取描述里最靠前的匹配。
- 覆盖 3511/3659 链 → **48 种车型**；未命中的 148 条主要是 `高峰线`（本就无车型）及个别国际/内燃怪例。
- ⚠️ 25 系**不带**可选前导数字：`225G`/`025G`/`525G` 那个多余首位数字是 `jlb.dat` 按控制字节分帧时漏进描述的噪声（数字字符 ≥0x20 不被切分），真实车型即 `25G`，去掉前导数字才能归一。
